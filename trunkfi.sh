...
if [[ -z $NEWBACKUP ]]; then
    if [[ $D_Hist -gt -1 ]]; then 
        ...
        $SSH "find "$RemoteDir" -maxdepth 1 -type d -name \"*.d\" -mtime +$D_Hist -print0 | xargs -0 -r -I file ${DRYRUN}mv file file.old"
    fi
    if [[ $W_Hist -gt -1 ]]; then
        PRINT "Searching for weekly backups older than $W_Hist weeks to delete..."
        $SSH "find "$RemoteDir" -maxdepth 1 -type d -name \"*.w\" -mtime +`expr $W_Hist \* 7` -print0 | xargs -0 -r -I file ${DRYRUN}mv file file.old"
    fi
    if [[ $M_Hist -gt -1 ]]; then
        PRINT "Searching for monthly backups older than $M_Hist weeks to delete..."
        $SSH "find "$RemoteDir" -maxdepth 1 -type d -name \"*.m\" -mtime +`expr $M_Hist \* 30` -print0 | xargs -0 -r -I file ${DRYRUN}mv file file.old"
    fi
    if [[ $Y_Hist -gt -1 ]]; then
        PRINT "Searching for yearly backups older than $Y_Hist years to delete..."
        $SSH "find "$RemoteDir" -maxdepth 1 -type d -name \"*.y\" -mtime +`expr $Y_Hist \* 365` -print0 | xargs -0 -r -I file ${DRYRUN}mv file file.old"
    fi
