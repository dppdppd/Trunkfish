...
#==============================================================================
#  Copyright 2011-2012 Ido Magal. All rights reserved. 
#==============================================================================
...
function setup_ssh(){
    ...
    if [[ 0 -ne `ssh ${ServerUser}@${Server} test -f "$AuthorizedKeys" ;echo -e \$?` ]]; then
        ...
        echo -e
        ${DRYRUN_}"$SSH" "mkdir ~/.ssh" 2> /dev/null
        echo -e
        echo -e "\t Creating $AuthorizedKeys on $Server."
        echo -e
        ${DRYRUN_}ssh ${ServerUser}@${Server} "touch "$AuthorizedKeys""
        echo -e
    else
        echo -e
    ...
    if [[ 0 -eq `eval test -f $SSHKeyPath.pub; echo \$?` ]]; then
        ...
        echo -e 
        ${DRYRUN_}ssh-keygen -t rsa -N '' -f "$SSHKeyPath"
    fi
    
    ${DRYRUN_}rsync --rsync-path="$RsyncPath" -e 'ssh' "$SSHKeyPath".pub ${ServerUser}@${Server}:~/.ssh/tmp
    ${DRYRUN_}ssh ${ServerUser}@${Server} "echo >> "$AuthorizedKeys" && cat ~/.ssh/tmp >> "$AuthorizedKeys" && rm ~/.ssh/tmp"
...
function reset_launchd(){
    ${DRYRUN_}launchctl stop com.trunkfish.backup 2>/dev/null
    ${DRYRUN_}launchctl unload "$PlistPath" 2>/dev/null
}
...
function schedule_launchd(){
    ...
    reset_launchd
    ${DRYRUN_}launchctl load "$PlistPath"
    ${DRYRUN_}launchctl start com.trunkfish.backup
}
...
function trap_backup() {
    ...
    ${DRYRUN_}"$SSH" "mv ${RemotePath}.incomplete ${RemotePath}.aborted.$$" &>/dev/null
    ${DRYRUN_}"$SSH" "nohup rm -r -f ${RemotePath}.aborted.$$ &" &>/dev/null

    # save the logs of the failed backup  
    ${DRYRUN_}mv $StdLogPath $ScriptDir"/"~TRUNKFISH_FAILED."${Today}".log &>/dev/null
    ${DRYRUN_}mv $StdErr $ScriptDir"/"~TRUNK_ERR_FAILED."${Today}".log &>/dev/null
    ${DRYRUN_}rm "$LockPath" &>/dev/null
    exit 1
}
#==============================================================================
...
# Abort if we can't find the settings file
...
for var in "$@"; do
    case "$var" in
        ...
        --dry-run)      DRYRUN_="echo DRY RUN: -- "; NOLOGS=1;;
...
#   24 # Partial transfer due to vanished source files
    ...
    ${DRYRUN_}rsync "${RsyncOptions[@]}" "$BackupDir" ${ServerUser}@${Server}:${RemotePath}.incomplete/
    trap - "${RsyncErr[@]}"
...
# backup was successful. Rename it with an appropriate extension (d, w, m, or y)
    ...
    ${DRYRUN_}$SSH "mv ${RemotePath}.incomplete ${RemotePath}.$Ext"

# Backup is done. Copy the logs into the backup dir.
    if [[ -z $NOLOGS ]]; then
        PRINT "Copying the logs..."
        ${DRYRUN_}rsync --rsync-path="$RsyncPath" -e "$RsyncSSH" "$StdLogPath" ${ServerUser}@${Server}:${RemotePath}.$Ext/
        ${DRYRUN_}rsync --rsync-path="$RsyncPath" -e "$RsyncSSH" "$ErrLogPath" ${ServerUser}@${Server}:${RemotePath}.$Ext/
    fi
